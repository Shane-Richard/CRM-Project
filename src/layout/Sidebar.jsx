  ...
  return (
    <>
      <aside className="fixed left-0 top-0 h-screen w-20 bg-[#0a0a0b] border-r border-white/5 flex flex-col z-50 items-center py-6 shadow-[4px_0_24px_rgba(0,0,0,0.3)]">
        ...
        <nav className="flex-1 w-full px-3 space-y-3 overflow-y-auto no-scrollbar flex flex-col items-center">
          {menuItems.map((item) => {
            ...
            const showBadge = item.badge && item.badge > 0 && !isActive;
            return (
              <button
                key={item.name}
              ...
              >
                ...
                {/* Unread Badge */}
                {showBadge && (
                  <div className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center shadow-lg shadow-red-500/40 animate-pulse border border-[#0a0a0b]">
      ...
      {hoveredItem && hoveredRect && (
        <div
          className="fixed px-3 py-2 bg-slate-900/90 backdrop-blur-md text-white text-[11px] font-bold uppercase tracking-widest rounded-xl border border-white/10 shadow-2xl pointer-events-none z-[100] animate-in fade-in slide-in-from-left-2"
          style={{
        ...
        >
          {hoveredItem}
          <div className="absolute left-0 top-1/2 -translate-x-1 -translate-y-1/2 w-2 h-2 bg-slate-900 border-l border-b border-white/10 rotate-45" />
        </div>
      )}
    </>
  );
